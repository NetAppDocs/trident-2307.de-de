...
* Ein häufiges Problem, das bei der Verwendung von Trident mit https://azure.microsoft.com/en-us/products/netapp/["Azure NetApp Dateien"] auftreten kann, besteht darin, dass die Mandanten- und Client-Secrets aus einer App-Registrierung mit unzureichenden Berechtigungen stammen. Eine vollständige Liste der Trident Anforderungen ist unter link:trident-use/anf.html["Azure NetApp Dateien"] Konfiguration zu finden.
